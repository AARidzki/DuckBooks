...
<div class="col-lg-8">
    <form method="POST" action="/dashboard/discounts/{{ $discount->id }}" class="mb-5" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="mb-3">
            ...
            <select class="form-select" name="books_id">
                @foreach ($books as $book)
                    @if (old('books_id', $discount->book_id) == $book->id)
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('kode_promo') is-invalid @enderror" id="kode_promo" name="kode_promo"
                required autofocus value="{{ old('kode_promo', $discount->kode_promo) }}">
            @error('kode_promo')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('promo') is-invalid @enderror" id="promo" name="promo"
                required autofocus value="{{ old('promo', $discount->promo) }}">
            @error('promo')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('expire') is-invalid @enderror" id="expire" name="expire"
                required autofocus value="{{ old('expire', $discount->expire) }}">
            @error('expire')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('count') is-invalid @enderror" id="count" name="count"
                required autofocus value="{{ old('count', $discount->count) }}">
            @error('count')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
